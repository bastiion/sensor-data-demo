...
/**
 * Generates JSON Schema for sensor data structures
 * Uses Zod 4's built-in JSON Schema support
 */
export const generateSensorJSONSchemas = () => {
  return {
    pointGeometry: toJSONSchema(PointGeometrySchema),
    sensorProperties: toJSONSchema(SensorPropertiesSchema),
    sensorFeature: toJSONSchema(SensorFeatureSchema),
    sensorFeatureCollection: toJSONSchema(SensorFeatureCollectionSchema),
  };
};
